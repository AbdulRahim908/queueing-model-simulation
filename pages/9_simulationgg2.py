...
def ServerUtilization(Server_util):
    ...
    st.pyplot()
def gg2(lembda,meu,sigma,ran_no):
    ...
    for x in range(0,ran_no):
        s_no.append(x)
        value=value+norm.pdf(x, meu, sigma)
    ...
    for i in range(ran_no):
        ran_var=float("%.4f"%random.uniform(0,0.1))
        #print(ran_var)
...
meu=st.number_input("meu minimum",step=1.,format="%.2f")
sigma=st.number_input("meu maximum",1,50)
ran_no=st.number_input('random numbers', 1,50)
df,s_no,arrival,service,WT,RT,TA,server1_util,server2_util,avg_interarrival,avg_service,avg_TA,avg_WT,avg_RT=gg2(lembda,meu,sigma,ran_no)
st.write(df)
